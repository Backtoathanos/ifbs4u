...
class order extends dbcon{
	...
	public function actiononOrder($order_id){
		...
		$product_details_result=mysqli_query($this->dbs, "
														...
														WHERE `fbs_order`.`order_id` = '".$order_id."' AND `fbs_order`.`status`='unconfirm'
														");
		$sl=1;
		...
		$result=mysqli_query($this->dbs, "
			SELECT * FROM `fbs_order` JOIN `fbs_customer` ON `order_jid`=`jid` WHERE `order_id`= '".$order_id."' AND `status`='unconfirm' AND `order_emp_id`= '0'
			");
		$op='
				...
				$op.='
					<form class="customer_order_not_deliver">
						<tbody >
							...
							// <tr>
							// 	<td colspan="2">
							// 		<input type="submit" class="btn btn-warning confirm" id="'.$row["order_id"].'" name="confirm" value="Confirm">
